...
set.seed(0)
df.sw.l$xjitter <-jitter(as.numeric(df.sw.l$cyclic) + (0.45 * as.numeric(df.sw.l$delay_cond) -.675))
...
ggplot(df.sw.l, aes(x=cyclic, y=final_acc_cy_ncy, fill=delay_cond)) +
  ...
  stat_summary(data = df.mod.s, aes(x=cyclic, fill=delay_cond, y=bayes.acc),
               fun.y=mean, geom="point", size=3, color='black',
               shape=17, position = position_dodge(width = .9), show.legend=F) +
  geom_point(aes(x=xjitter), alpha = .3) +
  geom_hline(yintercept = 1/4, color='black', linetype='dashed') +
  labs(x='',y='Accuracy', fill='Condition') +
  theme_bw() +
  coord_cartesian(y=c(0,1)) +
  theme(panel.grid = element_blank(),
      legend.position = 'bottom') +
  ggsave('../../figures/for_cogsci_talk/acc_cond_cyclic.pdf', width = 4, height=4)
...
################################
#Intervention scatters
names(df.sw)

ggplot(data = df.sw, aes(y=final_acc, x=int_space.mean/1000, shape = delay_cond)) +
  geom_point() +
  theme_bw() +
  labs(y='Accuracy',x='Action-Action Spacing (s)') +
  theme(legend.position = c(.7,.15),
        legend.title= element_blank(),
        panel.grid = element_blank()) +
  ggsave('../../figures/for_cogsci_talk/acc_space_scatter.pdf', width = 3, height = 3)

ggplot(data = df.sw, aes(y=final_acc, x=int_space_event.mean/1000, shape = delay_cond)) +
  geom_point() +
  theme_bw() +
  labs(y='Accuracy',x='Event-Action Spacing (s)') +
  theme(legend.position = c(.7,.15),
        legend.title= element_blank(),
        panel.grid = element_blank()) +
  ggsave('../../figures/for_cogsci_talk/acc_space_event_scatter.pdf', width = 3, height = 3)

df.sw$int_space.cv<-(df.sw$int_space.sd/df.sw$int_space.mean) 
df.sw$int_space_event.cv<-(df.sw$int_space.sd/df.sw$int_space_event.mean) 

ggplot(data = df.sw, aes(y=final_acc, x=1/int_space.cv, shape = delay_cond)) +
  geom_point() +
  theme_bw() +
  labs(y='Accuracy',x='Action-Action Regularity (1/CV)') +
  theme(legend.position = c(.7,.15),
        legend.title= element_blank(),
        panel.grid = element_blank()) +
  ggsave('../../figures/for_cogsci_talk/acc_cv_scatter.pdf', width = 3, height = 3)


ggplot(data = df.sw, aes(y=final_acc, x=1/int_space_event.cv, shape = delay_cond)) +
  geom_point() +
  theme_bw() +
  labs(y='Accuracy',x='Event-Action Regularity (1/CV)') +
  theme(legend.position = c(.7,.15),
        legend.title= element_blank(),
        panel.grid = element_blank()) +
  ggsave('../../figures/for_cogsci_talk/acc_cv_event_scatter.pdf', width = 3, height = 3)


ggplot(data = df.sw, aes(y=final_acc, x=int_pref_parent, shape = delay_cond)) +
  geom_point() +
  theme_bw() +
  labs(y='Accuracy',x='Root pref Mean(N child/N parent)') +
  theme(legend.position = c(.7,.15),
        legend.title= element_blank(),
        panel.grid = element_blank()) +
  ggsave('../../figures/for_cogsci_talk/acc_pt_scatter.pdf', width = 3, height = 3)


#########################
#Model fit results plots
head(mod_preds)


chosen_mod_names<-c('mostrecent', 'mostlikely', 'morelikely', 'bayes')
which_max_acc<-c()
all_accs<-all_perfs<-matrix(0, 0, 4)
for (id in unique(mod_preds$upi))
{
  df.i<-filter(mod_preds, upi==id, initial_judgment==F, bayes.exist==T)
  accs<-colSums(select(df.i, which(names(df.i) %in% paste(chosen_mod_names, '.cor', sep=''))))/sum(df.i$check)
  all_accs<-rbind(all_accs, accs)
  
  perf<-colSums(select(df.i, which(names(df.i) %in% paste(chosen_mod_names, '.perf', sep=''))))/sum(df.i$check)
  all_perfs<-rbind(all_perfs, perf)
  
  which_max_acc<-c(which_max_acc, which.max(accs))
  print(which(accs==max(accs)))
}
summary(factor(which_max_acc, levels = 1:4, labels = chosen_mod_names))



#Model performance
df<-data.frame(cbind(rep(1/4, 40), all_perfs), df.sw$final_acc, df.sw$delay_cond)
names(df)<-c('Random', 'Order', 'Time', 'Struc+Time', 'Ideal', 'Participant', 'Condition')
df<-df %>% gather(Model, Performance, Random:Participant) %>%
  mutate(Model = factor(Model, levels = c('Random', 'Order', 'Time', 'Struc+Time', 'Ideal', 'Participant')))

ggplot(df, aes(x=Model, y=Performance, colour = Condition)) +
  stat_summary(fun.data = mean_cl_normal, geom = 'errorbar', width = .1, position = position_dodge(.3)) +
  stat_summary(fun.y = mean, geom = 'point', position = position_dodge(.3)) +
  labs(y='Accuracy', x='Model', fill='Condition') +
  theme_bw() +
  theme(legend.position = c(.75,.25),
        panel.grid = element_blank()) +
  ggsave('../../figures/for_cogsci_talk/model_perf.pdf', width = 4, height = 3)

#Model correspondence
df<-data.frame(cbind(rep(1/4, 40), all_accs), df.sw$delay_cond)
names(df)<-c('Random', 'Order', 'Time', 'Struc+Time', 'Ideal',  'Condition')
df<-df %>% gather(Model, Correspondence, Random:Ideal) %>%
  mutate(Model = factor(Model, levels = c('Random', 'Order', 'Time', 'Struc+Time', 'Ideal')))

ggplot(df, aes(x=Model, y=Correspondence, colour = Condition)) +
  stat_summary(fun.data = mean_cl_normal, geom = 'errorbar', width = .1, position = position_dodge(.3)) +
  stat_summary(fun.y = mean, geom = 'point', position = position_dodge(.3)) +
  labs(y='Correspondence', x='Model', fill='Condition') +
  theme_bw() +
  theme(legend.position = c(.75,.25),
        panel.grid = element_blank()) +
  ggsave('../../figures/for_cogsci_talk/model_correspondence.pdf', width = 4, height = 3)

#Model N
df.sw$which_max_acc<-factor(which_max_acc, levels = 0:4,
                            labels = c('Random', 'Order','Time','Struc+Time','Ideal'))

ggplot(df.sw, aes(x=which_max_acc, fill = delay_cond)) +
  geom_bar(stat='count') +
  labs(y='N', x='Model', fill='Condition') +
  theme_bw() +
  theme(legend.position = 'bottom',
        panel.grid = element_blank()) +
  ggsave('../../figures/for_cogsci_talk/model_N.pdf', width = 4, height = 3)
