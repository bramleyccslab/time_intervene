...
load('../data/mf_results.rdata') #37 columns of model predictions
...
df.mod<- mod_preds %>% filter(final_judgment==T, bayes.exist==T, !is.na(bayes))
#50/720 (7%) trials end in NA's, almost entirely cyclic trials especially trial type '9'.
...
df.mod.s<-df.mod %>% group_by(delay_cond) %>%
  ...
  mutate(bayes.acc = sum_bayes/sum_check,
         delay_cond.f=factor(delay_cond, levels=c('reliable','within','between'), labels = c('reliable','variable-within','variable-between')))

df.sw$delay_cond.f<-factor(df.sw$delay_cond, levels=c('reliable','within','between'), labels = c('reliable','variable-within','variable-between'))

#A play performance by condition plot
ggplot(df.sw, aes(x=delay_cond.f, y=final_acc)) +
  geom_boxplot() +
  geom_hline(yintercept = 1/4, color='grey', linetype='dashed') +
  stat_summary(fun.y=mean, geom="point", size=3, show.legend = F) +
  stat_summary(data = df.mod.s, aes(x=delay_cond.f, y=bayes.acc), fun.y=mean, geom="point", size=3, color='red', shape=17, show.legend = F) +
  ...
  ggsave('../../figures/for_thesis/ti_acc_cond.pdf', width = 5, height=5)
...
df.sw.l$delay_cond.f<-factor(df.sw.l$delay_cond, levels=c('reliable','within','between'), labels = c('reliable', 'variable-within', 'variable-between'))
...
ggplot(df.sw.l, aes(x=delay_cond.f, y=final_acc_cy_ncy, fill=cyclic)) +
  geom_boxplot() +
  geom_hline(yintercept = 1/4, color='grey', linetype='dashed') +
  stat_summary(aes(y=final_acc_cy_ncy), fun.y=mean, geom="point", size=3, position = position_dodge(width = .75), show.legend=F) +
  #stat_summary(aes(y=final_acc), fun.y=mean, geom="point", size=5, shape = 18, show.legend=F) +
  stat_summary(data = df.mod.s, aes(x=delay_cond.f, y=bayes.acc),
  ...
  labs(x='Condition',y='Accuracy', fill='Cyclic') +
  theme_bw() +
  coord_cartesian(y=c(0,1)) +
  theme(panel.grid = element_blank(),
  ...
  ggtitle('Accuracy by condition') + 
  ggsave('../../figures/for_thesis/ti_acc_cond_cyclic.pdf', width = 5, height=5)
...
pdf('../../figures/for_thesis/subparts/ti_mod_acc.pdf', width=8, height=3)
...
pdf('../../figures/for_thesis/subparts/ti_acc_legend.pdf', width=3, height=1)
par(mar=c(3,1,1,1))
...
pdf('../../figures/for_thesis/subparts/ti_int_legend.pdf', width=3, height=1)
par(mar=c(3,1,1,1))
...
for (cond in c('reliable','within','between'))
{
  pdf(paste('../../figures/for_thesis/subparts/ti_mod_acc_', cond, '.pdf', sep=''), width=8, height=3)
  par(mar=c(1,1,1,1), mfrow=c(2,6))
  for (i in 1:12)
  {
    ix<-ord_ix[i]
    ...
    plot(Gfull, layout=locations,  edge.arrow.size=0)
    plot(G, layout=locations,  edge.arrow.size=1, add=T)
    text(0,-1.7,paste('Ints: ', round(sum(node_weights), digits=1), '\nAcc: ', round(100*mean(weights)), '%', sep=''))
  }
  dev.off()
}


pdf('../../figures/for_thesis/subparts/ti_int_cond_legend.pdf', width=3, height=1)
par(mar=c(3,1,1,1))
image(matrix(seq(0,1,length.out=100),100), col=intcols, yaxt='n', xaxt='n')
...
#   ggsave('../../figures/for_thesis/acc_by_dev.pdf', width = 10, height=5)
# 
# 
...
#   ggsave('../../figures/for_thesis/acc_by_dev_cond_edge.pdf', width = 10, height=10)
...
#   theme_bw() +
#   ggtitle('Accuracy by edge device and condition') + 
#   ggsave('../../figures/for_thesis/acc_by_dev_edge.pdf', width = 10, height=5)
...
ggplot(df, aes(x=Time, y=Component)) +
  ...
  ggsave('../../figures/for_thesis/subparts/blank_plot.pdf', width=6, height=2)
...
ggplot(df, aes(x=Time, y=Component)) +
  ...
  ggsave('../../figures/for_thesis/subparts/blank_plot4.pdf', width=6, height=3)
...
ggplot(df, aes(x=time, y=to)) +
  ...
  ggsave(filename = '../../figures/for_thesis/example_events_no_line.pdf',
         width=5, height=8)
ggplot(df, aes(x=time, y=to)) +
  geom_point( aes(fill=type), shape=21, size=2) +
  geom_segment(aes(x=cause_time, y=rev_from, xend=time, yend=rev_to), color='grey') +
  ...
  theme(legend.position = 'bottom') +
  ggtitle(paste('Participant ', i, ' ', df$delay_cond[1], sep='')) +
  ggsave(filename = '../../figures/for_thesis/example_events_line.pdf',
         width=5, height=8)
...
ggplot(df, aes(x=time, y=to)) +
  ...
  theme(legend.position = 'bottom') +
  #ggtitle(paste('Participant ', i, ' ', df$delay_cond[1], sep='')) +
  ggsave(filename = '../../figures/for_thesis/events_for_model_48_6.pdf',
         width=5, height=3)
...
ggplot(df, aes(x=time, y=to)) +
  ...
  theme(legend.position = 'bottom') +
  #ggtitle(paste('Participant ', i, ' ', df$delay_cond[1], sep='')) +
  ggsave(filename = '../../figures/for_thesis/within_example_line6.pdf',
         width=5, height=3)
...
ggplot(df2, aes(x=time, y=to)) +
  ...
  ggsave(filename = '../../figures/for_thesis/example_beliefs.pdf',
         width=5, height=8)
...
ggplot(df2, aes(x=time, y=to)) +
  ...
  theme(legend.position = 'bottom') +
  #ggtitle(paste('Participant ', i, ' ', df2$delay_cond[1], sep=''))     +
  ggsave(filename = '../../figures/for_thesis/belief_for_model_48_6.pdf',
         width=5, height=3)
...
{
  pdf(paste('../../figures/for_thesis/example_48_6_belief_', i, '.pdf',sep=''), width=5, height=5)
  par(mar=c(1,1,1,1))
...
ggplot(df, aes(x=Time, y=Component)) +
  ...
  ggsave('../../figures/for_thesis/ti_dependence_example.pdf', width=5, height=3)
...
ggplot(df, aes(x=Time, y=Component)) +
  ...
  theme(axis.text.x=element_blank()) +
  coord_cartesian(xlim=c(0,45000)) +
  ggsave('../../figures/for_thesis/ti_dependence_example2.pdf', width=5, height=3)
...
ggplot(df, aes(x=Time, y=Component)) +
  ...
  theme(axis.text.x=element_blank()) +
  coord_cartesian(xlim=c(0,45000)) +
  ggsave('../../figures/for_thesis/ti_dependence_example5.pdf', width=5, height=3)
...
ggplot(df, aes(x=Time, y=Component)) +
  ...
  theme(axis.text.x=element_blank()) +
  coord_cartesian(xlim=c(0,45000)) +
  ggsave('../../figures/for_thesis/ti_dependence_example10.pdf', width=5, height=3)
