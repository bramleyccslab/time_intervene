...
# {
test<-foreach(pptix=c(7,13,26,30,32,35)) %dopar% #1:length(upis)) %dopar%
{
  source('likelihood_functions.R')
  ...
  for (ttix in 1:12)
  {
    ...
    {
      events<-filter(df.ev, upi==upis[pptix], trial_type==ttix) %>%
  ...
  save(file=paste('../data/individual_fits_cogsci/ppt', pptix, 'upi', upis[pptix],  '.rdata',sep=''), likelihoods, posts)
